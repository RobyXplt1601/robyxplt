...
// Fungsi untuk menghapus folder rekursif (beserta isinya)
function delete_directory($dir) {
    ...
    foreach ($items as $item) {
        ...
        if (is_dir($path)) {
            delete_directory($path); // Rekursif hapus subfolder
        } else {
...
// Unzip
if ($action === 'unzip') {
    $zip_file = $current_dir . '/' . basename($_GET['file'] ?? '');
    if (is_file($zip_file) && strtolower(pathinfo($zip_file, PATHINFO_EXTENSION)) === 'zip') {
        $zip = new ZipArchive();
        if ($zip->open($zip_file) === TRUE) {
            // Ekstrak langsung ke current_dir TANPA membuat folder baru
            if ($zip->extractTo($current_dir)) {
                $zip->close();
                $msg = 'File ZIP berhasil diekstrak!';
            } else {
        ...
        $msg = 'File tidak ditemukan atau bukan file ZIP!';
    }
}
...
<body>
    ...
    <table>
        ...
        <tr>
            ...
            <td>-</td>
            <td>
                <a href="?dir=<?= urlencode($current_dir) ?>&action=delete&file=<?= urlencode($folder) ?>" onclick="return confirm('Hapus folder beserta isinya?')">Hapus</a> |
                <form method="post" style="display:inline;">
                    <input type="hidden" name="action" value="rename_folder">
                    <input type="hidden" name="target" value="<?= htmlspecialchars($folder) ?>">
                    <input type="text" name="new_name" value="<?= htmlspecialchars($folder) ?>" size="18">
        ...
        <tr>
            ...
            <td>
                ...
                <?php if (strtolower(pathinfo($file['name'], PATHINFO_EXTENSION)) === 'zip'): ?>
                    <a href="?dir=<?= urlencode($current_dir) ?>&action=unzip&file=<?= urlencode($file['name']) ?>" onclick="return confirm('Ekstrak file ZIP?')">Unzip</a> |
                <?php endif; ?>
